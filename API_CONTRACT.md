...
Implement `create_app(token, home, engine_factory=None, owner_limit=None, idle_exit=None, idle_seconds=0)` in `claude_native_bridge/api.py`. Default engine factory constructs `NativeBridgeClient(hermes_home=home)`. Expose authenticated `GET /health`, `GET /v1/models`, `POST /v1/chat/completions`, `POST /v1/owner/close`, `POST /v1/service/shutdown`, and a bounded server entry point (`claude_native_bridge.api_server --home ... --token-file ... --ready-file ... --port ...`), launched only through `runtime_environment.command` in the isolated server runtime. Bind only loopback. The parent owns installation, credential/config storage and service startup coordination.

Service lifecycle: authenticated requests may carry `X-Hermes-Bridge-Process: <pid>:<create_time>` and `X-Hermes-Bridge-Client: <owner>`. A client is open from the first request naming it (the creation-time `/health` probe) until `POST /v1/owner/close` from the same process identity, or until that process exits. Native owners of a client whose process exited without closing it are released by the service's reaper (checked every 30 seconds). `idle_exit` is the host's stop callback. With it, the service retires once it has been unused for `idle_seconds`, no live process holds an open client, and no request or native owner is busy. `POST /v1/service/shutdown` returns `202`, refuses new work, drains in-flight work for a bounded time, then stops. Without `idle_exit` it returns `409`. While draining, `/health` answers `503 {"service": "claude-native-bridge", "status": "draining"}` and other routes answer `503`.
