...
Service lifecycle: authenticated requests may carry `X-Hermes-Bridge-Process: <pid>:<create_time>` and `X-Hermes-Bridge-Client: <owner>`. A client is open from the first request naming it (the creation-time `/health` probe) until `POST /v1/owner/close` from the same process identity, or until that process exits. `idle_exit` is the host's stop callback. With it, the service retires once it has been unused for `idle_seconds`, no live process holds an open client, and no request or native owner is busy. `POST /v1/service/shutdown` returns `202`, refuses new work, drains in-flight work for a bounded time, then stops. Without `idle_exit` it returns `409`. While draining, `/health` answers `503 {"service": "claude-native-bridge", "status": "draining"}` and other routes answer `503`.
