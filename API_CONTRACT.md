...
Service lifecycle: authenticated requests may carry `X-Hermes-Bridge-Process: <pid>:<create_time>`. `idle_exit` is the host's stop callback. With it, the service retires once it has been unused for `idle_seconds`, no identified client process is alive, and no request or native owner is busy. `POST /v1/service/shutdown` returns `202`, refuses new work, drains in-flight work for a bounded time, then stops. Without `idle_exit` it returns `409`. While draining, `/health` answers `503 {"service": "claude-native-bridge", "status": "draining"}` and other routes answer `503`.
