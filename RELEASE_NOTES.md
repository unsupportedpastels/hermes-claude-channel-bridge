...
- **Lifecycle.** The shared service exits after `claude_native_bridge_api.idle_exit_seconds` (default 300) once no identified Hermes client process (PID plus start time) is alive and nothing is in flight. `stop` asks the service to drain and exit before falling back to identity-checked termination.
